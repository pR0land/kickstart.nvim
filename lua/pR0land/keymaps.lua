-- These are my custom keymaps for NEOVIM
...
vim.keymap.set('n', '<leader>tl', function()
  local langs = { 'en_gb', 'en_us' }
  local current = vim.o.spelllang
  local next = langs[(vim.fn.index(langs, current) + 1) % #langs + 1]
  vim.opt_local.spelllang = next
  vim.notify('CurrentLanguage → ' .. next)
end, { desc = '[t]oggle [l]anguage' })
